...
if (TARGET.upgrade)
{
    ...
    YY = XX;
    var upgrades = upgrade_tree(TARGET._tank);
    var len = array_length_1d(upgrades);
    var XXX = (XX - 20) / 2;
    var YYY = (YY - 20) / 2;
    var SurfaceAspect = 2;
    var InterpolateFactor = 2;
    
    for (i = 1; i < len; i++)
    {
        var sizey = 0.1 * SurfaceAspect * tank_get_size(upgrades[0] * 15);
        var _surf = surface_create((XXX * sizey) / 1.6, (YYY * sizey) / 1.6);
        ...
        draw_set_alpha(1);
        var gunplacing = set_gun(upgrades[i], 0);
        var gunnum = ds_list_size(gunplacing);
        var endpoints = ds_list_create();
        
        for (var j = 0; j < gunnum; j++)
        {
            var position = ds_list_find_value(gunplacing, j);
            var gunlength = position[0];
            var gunsetx = position[3];
            var gunangle = position[5];
        ...
        var middle = mean(ds_list_find_value(endpoints, 0), ds_list_find_value(endpoints, ds_list_size(endpoints) - 1));
        ds_list_destroy(endpoints);
        var XDRAW = (surface_get_width(_surf) / 2) - lengthdir_x((sizey * middle) / 2, rotation);
        var YDRAW = (surface_get_height(_surf) / 2) - lengthdir_y((sizey * middle) / 2, rotation);
        
        for (var j = 0; j < gunnum; j++)
        {
            var position = ds_list_find_value(gunplacing, j);
            var gunlength = position[0];
            var gunheight = position[1];
            var gunaspect = position[2];
            var gunsetx = position[3];
            var gunsety = position[4];
            var gunangle = position[5] + rotation;
            var gundistance = length(gunsetx, gunsety);
            var gundirection = point_direction(0, 0, gunsetx, gunsety) + gunangle;
            ...
            var theta = arctan((gunheight * (gunaspect - 1)) / gunlength);
            var yoffset1 = SurfaceAspect * 2 * tan(0.7853981633974483 - (theta / 2));
            var yoffset2 = SurfaceAspect * 2 * tan(0.7853981633974483 + (theta / 2));
            var h1, h2;
            ...
            draw_rotated_trapezoid(1 + XDRAW + (sizey * g_x), 1 + YDRAW + (sizey * g_y), sizey * gunlength, h1, h2, gunangle, 8947848, 1);
        }
        
        draw_set_color(merge_color(#3366D6, c_black, 0.4));
        draw_circle(XDRAW, YDRAW, (10 * sizey) + (2 * SurfaceAspect), 0);
        draw_set_color(#3366D6);
        draw_circle(XDRAW, YDRAW, 10 * sizey, 0);
        ds_list_destroy(gunplacing);
        surface_reset_target();
        var _surf_interpolate = surface_create(surface_get_width(_surf) * InterpolateFactor, surface_get_height(_surf) * InterpolateFactor);
        ...
        draw_text_diep((xx + xx + XXX) / 2, (yy + YYY) - 14, tank_get_name(upgrades[i]) + " [ " + get_upgrade_key(i) + " ]", 16777215, 7, 0, 1, 20);
...
var xplace = (xx - XX) / room_width;
var yplace = (yy - YY) / room_height;
var i = ds_map_find_first(MINIMAP);

if (!is_undefined(i))
{
    repeat (ds_map_size(MINIMAP))
    {
        var data = ds_map_find_value(MINIMAP, i);
        var newi = ds_map_find_next(MINIMAP, i);
        var XMAP = data[0];
        var YMAP = data[1];
...
draw_set_color(#888888);
draw_rectangle(xx, yy, XX, YY, false);
draw_set_color(#DBA015);
draw_rectangle(xx, yy, xx + ((XX - xx) * (displayed_score / displayed_score_level)), YY, false);
draw_set_color(merge_color(#888888, c_black, 0.4));
draw_rectangle(xx, yy, XX, YY, true);
draw_set_halign(fa_center);
draw_set_valign(fa_middle);
draw_text_diep((xx + XX) / 2, (yy + YY) / 2, "Level " + string(TARGET.skilllevel) + " " + tank_get_name(TARGET._tank), 16777215, BARHEIGHT - 4, 0, 1, 8);
YY -= (BARHEIGHT + 4);
BARHEIGHT = 10;
yy = YY - BARHEIGHT;
draw_set_color(#888888);
...
draw_set_halign(fa_center);
draw_set_valign(fa_middle);
draw_text_diep((xx + XX) / 2, (yy + YY) / 2, "Score: " + handle_large_number(TARGET.scorevalue), 16777215, BARHEIGHT - 2, 0, 1, 8);
BARHEIGHT = 11;
xx = 10;
XX = (alcovesize * 1000) - 10;
yy = 590 - BARHEIGHT;
...
for (var n = 9; n >= 0; n--)
{
    ...
    for (var j = 1; j < (TARGET.skill[i] + 1); j++)
    {
        var hotspot = xx + ((XX - xx) * (j / 10));
        draw_line(hotspot, yy, hotspot, YY);
    }
...
if (TARGET.skillpoints)
{
    ...
    draw_text_diep(XX + 2, yy, "x" + string(TARGET.skillpoints), textcolor, 12, 15, 1, 8);
}
