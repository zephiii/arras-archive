...
for (var i = 0; i < 2; i++)
{
    var n = instance_nth_nearest(x, y, 7, i + 2);
        ...
        if (!(component < (n_dist - combinedradius)))
        {
            ...
            if (goahead)
            {
                ...
                if (id.masterid != n.masterid)
                {
                    ...
                    var damCon = 1;
                    var resistdiff = my_Resist - n.my_Resist;
                    var mydamage = damCon * (1 + resistdiff) * my_Damage * max(speedfactor, 1) * speedfactor;
                    var n_damage = damCon * (1 - resistdiff) * n.my_Damage * max(n_speedfactor, 1) * n_speedfactor;
                    ...
                    var mybit = 1;
                    var n_bit = 1;
                    var stuff = get_health_damage(n_damagetoapply, my_Health, my_Resist);
                ...
                if (my_Pushability && n.my_Pushability)
                {
                    ...
                    {
                        var elasticity = 2 - ((4 * arctan(my_Penetration * n.my_Penetration)) / pi);
                        ...
                        var spring = sqrt(my_Ratio * n.my_Ratio);
                        ...
                        var force = -(elasticimpulse + springimpulse) * reductionfactor * my_Pushability * n.my_Pushability;
                        var mymodifiers = (1 - shield_level(my_MaxShield, my_Shield)) / mass;
                        var n_modifiers = (1 - shield_level(n.my_MaxShield, n.my_Shield)) / n.mass;
