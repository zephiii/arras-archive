...
var key_alt = mouse_check_button(mb_right);
...
var powercoeff = 1;
var move = (my_Acceleration * powercoeff) / power(move_mass, 0.25);
...
var vfrict = -my_Friction * yspeed;
var hfrict = -my_Friction * xspeed;
...
for (var g = 0; g < my_GunNumber; g++)
{
    ...
    var settings = ds_list_find_value(gunsettings, g);
    var g_type = settings[13];
    ...
    if (gunreload[g])
    {
        if (g_type != 0 || drone_count < my_maxDrones)
        {
            g_armed[g] -= min(1, g_armed[g]);
            
            while ((g_type != 0 && key_shoot && g_armed[g] < 1) || (g_type == 0 && drone_count < my_maxDrones && g_armed[g] < 1))
            {
