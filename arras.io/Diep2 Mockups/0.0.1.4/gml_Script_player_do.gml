var i, k, upgrades, len, key_shoot, key_alt, key_shoot_pressed, key_up, key_dn, key_lf, key_rg, move_mass, horz, vert, rate, powercoeff, move, realtopspeed, vfrict, hfrict, g, sizey, g_facing, recoilforce, gundistance, gundirection, g_x, g_y, settings, g_type, theta, yoffset1, yoffset2, h1, h2;
...
key_alt = mouse_check_button(mb_right)
key_shoot_pressed = mouse_check_button_pressed(mb_left)
if ((!can_lock_direction) || (!key_alt))
    facing = point_direction(x, y, mouse_x, mouse_y)
if (can_control_drones && (key_shoot || key_alt))
{
    if key_shoot
        target_active = 1
    else
...
powercoeff = 1
move = ((my_Acceleration * powercoeff) / power(move_mass, 0.25))
...
if (vert && horz)
    move *= 0.7071
vfrict = ((-my_Friction) * yspeed)
hfrict = ((-my_Friction) * xspeed)
if vert
{
    if key_up
    {
...
for (g = 0; g < my_GunNumber; g++)
{
    ...
    settings = ds_list_find_value(gunsettings, g)
    g_type = settings[13]
    switch g_type
    {
        case Swarm:
    ...
    if gunreload[g]
    {
        if (g_type != Drone || drone_count < my_maxDrones)
        {
            g_armed[g] -= min(1, g_armed[g])
            while ((g_type != 0 && key_shoot && g_armed[g] < 1) || (g_type == Drone && drone_count < my_maxDrones && g_armed[g] < 1))
            {
                g_motion[g] -= (4 * ln(((stat_Speed * gunrecoil[g]) + 0.01)))
                shoot_bullet((x + (sizey * g_x[g])), (y + (sizey * g_y[g])), g_type, g_facing[g], my_Size, gunheight[g], g_armed[g], settings, array(stat_Damage, stat_Penetration, stat_Speed, stat_Strength, stat_BulletResist))
