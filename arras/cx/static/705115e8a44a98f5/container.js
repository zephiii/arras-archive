...
var ma={h:"adinplay",o:!1,show(a=this.h){this.o&&a!==this.h&&this.u();this.o=!0;this.h=a;"adinplay"===this.h?ia().cmd.display.push(()=>window.aipDisplayTag.display("arras-io_336x280")):"nitropay"===this.h?(J("ad-spawn","block"),ja().createAd("ad-spawn",{refreshLimit:10,refreshTime:30,renderVisibleOnly:!1,refreshVisibleOnly:!0,sizes:[[336,280],[300,250]]})):"bsa"===this.h?(J("bsa-zone_1643828555140-9_123456","block"),ka().queue.push(()=>window.optimize.push("bsa-zone_1643828555140-9_123456"))):J("referral-fallback",
"block")},u(){this.o=!1;"adinplay"===this.h?ia().cmd.display.push(()=>window.aipDisplayTag.clear("arras-io_336x280")):"nitropay"===this.h?J("ad-spawn","none"):"bsa"===this.h?J("bsa-zone_1643828555140-9_123456","none"):J("referral-fallback","none")}},na={h:"adinplay",o:!1,show(a=this.h){this.o&&a!==this.h&&this.u();this.o=!0;this.h=a;J("respawn-banner","block");if("adinplay"===this.h)ia().cmd.display.push(()=>window.aipDisplayTag.display("arras-io_728x90"));else if("nitropay"===this.h||"nitropay-mobile"===
...
refreshVisibleOnly:!0,sizes:a})}else"bsa"===this.h&&(J("bsa-zone_1643827950431-2_123456","block"),ka().queue.push(()=>window.optimize.push("bsa-zone_1643827950431-2_123456")))},u(){this.o=!1;if("adinplay"===this.h)ia().cmd.display.push(()=>window.aipDisplayTag.clear("arras-io_728x90"));else if("nitropay"===this.h||"nitropay-mobile"===this.h){let a=document.getElementById("ad-respawn");a&&a.remove()}else"bsa"===this.h&&J("bsa-zone_1643827950431-2_123456","none");J("respawn-banner","none")},V(){let a=
...
(()=>{if(!va||xa){try{window.top.location="https://web.archive.org/web/20221203104346/https://arras.io/";return}catch(b){}let c=!1;document.addEventListener("click",()=>{if(!c){c=!0;try{window.top.location="https://web.archive.org/web/20221203104346/https://arras.io/"}catch(b){}}})}if("http:"===location.protocol||location.hostname.startsWith("www."))location.href=`https://${location.host.replace(/^www\.|:80$/g,"")}/${"#"===location.hash?"":location.hash}`;else{var a=!1;window.addEventListener("error",c=>{if(!a){a=!0;var {message:b,filename:d,lineno:g,colno:e,error:h}=c;h&&(h=h.toString());
...
fetch(`/probe?id=${Math.random().toString(36).slice(2)}&text=ad-prebid-ad-300x250-ad-unit.js`).then(a=>a.json()).catch(()=>({ok:!1})).then(a=>{N=!a.ok&&!pa.F;v||(N?ma.show("fallback"):pa.F||setTimeout(()=>{pa.F||ma.show("fallback")},5E3));ya({type:"initial",user:{adblock:N,mobile:v,window:{innerWidth:window.innerWidth,innerHeight:window.innerHeight},tracking:{name:L.get("name")||"",colors:L.get("colors")||"normal",borders:L.get("borders")||"normal"}}})});var Ca=L.get("legacyToken")||null;
let Da=parseInt(L.get("privilege"),10),Ea=Number.isNaN(Da)?Ca?1:0:Da,O=!1,Fa=[{z:["Private",null],x:["Local",null],glitch:["Glitch",10],buyvm:["BuyVM",15],extravm:["ExtraVM",40],hetzner:["Hetzner",50],itc:["Incubatec",45],ovh:["OVH",35],wsi:["WSI",50],vultr:["Vultr",30]},{xyz:["Local","Localhost",null],unk:["Unknown","Unknown",null],svx:["US West","Silicon Valley, CA, US",-7],lax:["US West","Los Angeles, CA, US",-7],dal:["USA","Dallas, TX, US",-5],kci:["USA","Kansas City, MO, US",-5],vin:["US East",
...
d=d.host;g&&(d=g,d.includes(";")&&(d=d.split(";").shift()));d&&(location.href=`https://web.archive.org/web/20221203104346/https://arras.cx/#${a}`);return null}return R.find(e=>e.id===c)||null},Ia=a=>{if(!a||"%"===a)return"Unknown";var c=Fa[2];let b=[],d=[];var g=0;for(let n of c)for(let f of n)if(f.id===a.charAt(g)){g++;c=Object.assign({},f);if("word"===f.G){var e=parseInt(a.charAt(g++),36),h=a.slice(g,g+e);c.g=h.charAt(0).toUpperCase()+h.slice(1);g+=e}else if("words"===f.G){e=parseInt(a.charAt(g++),36);h=[];for(let k=0;k<e;k++){var l=a.charAt(g++);
if("d"===l)h.push("-");else if("s"===l)h.push(" ");else{l=parseInt(l,36);let m=a.slice(g,g+l);h.push(m.charAt(0).toUpperCase()+m.slice(1));g+=l}}c.g=h.join("")}f.remove&&(Array.isArray(f.remove)?d.push(...f.remove):d.push(f.remove));b.push(c);break}if(0===b.length)return"Unknown";a=b[b.length-1];a.end&&(a.g=a.end,a.advance&&(a.advance=!1));for(a=0;a+1<b.length;a++)b[a].delay&&b[a+1].advance&&(g=b[a],b[a]=b[a+1],b[a+1]=g,a++);b=b.filter(({id:n})=>!d.includes(n));return b.map(n=>n.g).join(" ")},Ja=
...
d=g,!a)){a=!0;var e=["https://web.archive.org/web/20221203104346/https://arrax.io/","https://web.archive.org/web/20221203104346/https://arras.netlify.app/","https://web.archive.org/web/20221203104346/https://sites.google.com/view/arras-io","https://web.archive.org/web/20221203104346/https://arras.cx/"];for(let h=0;h<e.length;h++){let l=document.getElementById(`proxy-link-${h}`);l.href=e[h];l.innerText=e[h].replace(/^https:\/\/|\/$/g,"")}}})})();
...
{if(!x){var G=g.getBoundingClientRect();if(!(0>=G.height)){var I=Ra.getBoundingClientRect();G.top>I.bottom+I.height||G.bottom<I.top-I.height||(x=!0,z())}}};Ra.addEventListener("scroll",B);setTimeout(B,50)})).then(()=>fetch(`${"https://web.archive.org/web/20221203104346/https://poll-server.arras.cx:2020"}/poll/${k}/status`)).then(z=>z.json()).then(z=>{if(!z.ok)throw Error("Poll does not exist");Q=z.options.map(x=>({S:x?x.voted:!1,T:x?x.votes:0}))});H.addEventListener("click",()=>{H.remove();q.remove();let z=A.map(B=>parseInt(B.title,10)).sort((B,
...
- A post regarding upcoming content has been published at <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.reddit.com/r/Arrasio/comments/ximykz/whats_next_for_arras/">redd.it/ximykz</a>.
...
# Update [2022-09-20]
- There is now a new proxy link at <a target="_blank" href="https://web.archive.org/web/20221203104346/https://sites.google.com/view/arras-io">https://sites.google.com/view/arras-io</a>, which allows you to access the game when the main domain is unavailable.
- Private servers have now been moved to use <a target="_blank" href="https://web.archive.org/web/20221203104346/https://arras.cx/">https://arras.cx/</a> as the link. A final update for private servers has also been published on <a target="_blank" href="https://web.archive.org/web/20221203104346/https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> to fix a bug with shield bars.
- For now, <a target="_blank" href="https://web.archive.org/web/20221203104346/https://arras.cx/">https://arras.cx/</a> can also function similar to a regular proxy link, but in the future, it will be deprecated and used for private servers only.
...
# Update [2022-02-01]
- There is now a new proxy link at <a target="_blank" href="https://web.archive.org/web/20221203104346/https://arrax.io/">arrax.io</a>, which allows you to access the game when the main domain is unavailable.
...
- Note that if you own a private server, you can now go <a target="_blank" href="https://web.archive.org/web/20221203104346/https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> to enable the class tree.
...
- Added support for <a href="https://web.archive.org/web/20221203104346/https://arras.io/">HTTPS</a>.
...
# Patch [2020-03-09]
- Added a button that links to <a target="_blank" href="https://web.archive.org/web/20221203104346/https://momentumstudios.games/">Momentum Studios</a>.

# Patch [2020-03-09]
- Migrated from <a target="_blank" href="https://web.archive.org/web/20221203104346/https://surge.sh/">surge.sh</a> to <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.netlify.com/">Netlify</a>.
...
- Added <a target="_blank" href="https://web.archive.org/web/20221203104346/https://wikipedia.org/wiki/Reaction_control_system">RCS</a> that will automatically make you slow down if you don't press any key, which you can disable with R.
...
- Note that if you own a private server, you need to go <a target="_blank" href="https://web.archive.org/web/20221203104346/https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> to keep your server working!
- Updated the <a target="_blank" href="https://web.archive.org/web/20221203104346/https://glitch.com/edit/#!/arras-template?path=README.md">link format for private servers</a>.
...
- Added a way to <a target="_blank" href="https://web.archive.org/web/20221203104346/https://glitch.com/edit/#!/arras-template?path=README.md">specify gamemodes for private servers</a>.
...
- Note that if you own a private server, you need to go <a target="_blank" href="https://web.archive.org/web/20221203104346/https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> and update your server accordingly to keep your server working!
...
- Added <a href="https://web.archive.org/web/20221203104346/https://youtu.be/5XY8kKIZN-w">custom keybinds</a>, which you can access by going to the <a href="#options-menu">options menu</a> and clicking on the key indicators.
...
# Gamemode [2019-04-30]
- Added a new server in Asia, also sponsored by <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.serverhunter.com/?utm_source=arrasio&utm_medium=button&utm_content=website">Server Hunter</a>.
- Added Maze on the server.
- Changed the OVH Europe server to FFA.

# Gamemode [2019-04-20]
- Added a new US East server from OVH sponsored by <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.serverhunter.com/?utm_source=arrasio&utm_medium=button&utm_content=website">Server Hunter</a>.
...
- This server is also sponsored by <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.serverhunter.com/?utm_source=arrasio&utm_medium=button&utm_content=website">Server Hunter</a> and should be faster.

# Gamemode [2019-04-05]
- Added a new European server from Hetzner.
- Thanks to <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.serverhunter.com/?utm_source=arrasio&utm_medium=button&utm_content=website">Server Hunter</a> for sponsoring it!
- Currently the gamemode is Maze but I'll likely change the gamemode for it in the future.
- I've replaced the link for <a href="https://web.archive.org/web/20221203104346/https://arras-lb.glitch.me/donate">Stripe donation</a>, but you can still access it.
...
- Fixed a memory leak that caused server crashes, credit to <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.reddit.com/user/CrazyDave2345">&.</a> for telling me to investigate it.
...
- Thanks to all the donors of the game, we now have a new domain for the game at <a target="_blank" href="https://web.archive.org/web/20221203104346/http://arras.io/">arras.io</a>!
...
- Added the <a href="https://web.archive.org/web/20221203104346/https://arras-lb.glitch.me/donate">one-time donation link</a> to the options menu.
...
- In order to keep all of the servers up at all time, you can donate to me on <a href="https://web.archive.org/web/20221203104346/https://www.patreon.com/arras">Patreon</a> so I can upgrade the US West server to have more bandwidth.
...
# Patch [2018-11-18]
- Added a <a target="_blank" href="https://web.archive.org/web/20221203104346/http://arras-proxy.surge.sh/">proxy server</a> because some people can't seem to play on the new link.
...
# Patch [2018-11-05]
- Added back minimap to show teammates and bosses, credit to <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.reddit.com/user/CrazyDave2345">&.</a> for helping to make it faster.
- Note that if you own a private server, you need to go <a target="_blank" href="https://web.archive.org/web/20221203104346/https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> to keep your server working!
...
# Update [2018-10-31]
- Added Pumpkin Skeleton theme, credit to <a target="_blank" href="https://web.archive.org/web/20221203104346/https://www.reddit.com/user/Road-to-100k">Road</a>.
...
- Thanks to our patrons on <a href="https://web.archive.org/web/20221203104346/https://www.patreon.com/arras">Patreon</a>, we now have a new domain for the game at <a target="_blank" href="https://web.archive.org/web/20221203104346/http://arras.cx/">arras.cx</a>!
...
# Gamemode [2018-08-06]
- Added 2TDM and 4TDM!
- Thanks to our patrons on <a href="https://web.archive.org/web/20221203104346/https://www.patreon.com/arras">Patreon</a> for allowing us a second server!
...
# Update [2018-07-02]
- Various balance changes.
- If you have a private server, make sure to go <a target="_blank" href="https://web.archive.org/web/20221203104346/https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> and do the required changes to keep your server working!
...
- Added <a href="https://web.archive.org/web/20221203104346/https://www.patreon.com/arras">Patreon</a> page!
...
c.fillRect(0,0,a.width,a.height),c.lineWidth=2.5,c.lineCap="round";c.globalCompositeOperation="destination-out";c.fillStyle="rgba(0,0,0,0.15)";c.fillRect(0,0,a.width,a.height);c.globalCompositeOperation="lighter";for(var e of d){var h=e.x,l=e.y;e.A+=.2;e.x+=e.D;e.y+=e.A;e.A*=.99;e.D*=.99;e.time--;var n=0<e.time?e.v?1:10<=e.time?1:e.time/10:0;if(0<n)c.strokeStyle=`rgba(${e.color},${n})`,c.beginPath(),c.moveTo(h,l),c.lineTo(e.x,e.y),c.stroke();else{if(e.v&&!e.s){h=Math.floor(5*Math.random())+30;l=.5*
Math.random()+3;n=25+5*Math.random();for(var f=0;2>f;f++){let k=b();for(let m=0;m<h;m++){let r=(m+Math.random())/h*Math.PI*2,D=l+.5*Math.random();d.push({color:k,x:e.x,y:e.y,D:Math.cos(r)*D,A:-.8+Math.sin(r)*D,time:n+2*Math.random(),v:!1,s:!1})}}}e.s=!0}}3E-5*a.width>Math.random()&&(e=a.width*Math.random(),h=a.height-10,l=4*Math.random()-2,n=5*Math.random()-15,f=30+10*Math.random(),d.push({color:b(),x:e,y:h,D:l,A:n,time:f,v:!0,s:!1}));O?a.remove():qa(g)};setInterval(()=>{d=d.filter(e=>!e.s)},2E3);
g()};}()

}

/*
     FILE ARCHIVED ON 10:43:46 Dec 03, 2022 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 15:35:05 Jul 27, 2026.
...
playback timings (ms):
  ...
  load_resource: 506.462
  PetaboxLoader3.resolve: 406.0
  PetaboxLoader3.datanode: 18.824
*/